...
func main() {
	...
	if len(os.Args) < 2 {
		fmt.Println("Usage: skyip <command> -f/--config <path> [-v|--version]")
		fmt.Println("\nCommands:")
		fmt.Println("  run   - Execute the dynamic DNS update (for cron/systemd). Usage: skyip run -f <path> [-i]")
		os.Exit(1)
	}
	...
	default:
		...
		fmt.Println("Usage: skyip run -f/--config <path> [-i]")
		os.Exit(1)
	}
}
