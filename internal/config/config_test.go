...
func TestValidateConfig(t *testing.T) {
	tests := []struct {
				...
				Records: []RecordConfig{
					{
						...
						Cloudflare: &CloudflareRecord{
							APIToken: "test_token_12345678901234567890",
						},
					},
				},
			},
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "aliyun access_key_id and access_key_secret are required",
		},
		{
...
func TestGetCacheFilePath(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "with_work_dir",
			configFile: "/etc/goddns/config.json",
			workDir:   "/var/lib/goddns",
